...
export function ResultadosBusqueda({ origen, destino, fecha }: { origen: string; destino: string; fecha: string }) {
  ...
  if (!origen || !destino || !fecha) {
    return <p className="text-slate-500">Elige origen, destino y fecha para buscar.</p>;
  }
  if (isPending) return <p className="text-slate-500">Buscando viajes...</p>;
  if (error) return <p className="text-red-600">No se pudo buscar: {error.message}</p>;
  if (resultados.length === 0) {
    return <p className="rounded-lg bg-white p-4 text-slate-600">No hay viajes disponibles para esa fecha. Prueba con otro dia.</p>;
  }
        ...
        <li key={`${r.viaje_id}-${r.origen.orden}-${r.destino.orden}`} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-1">
            <p className="text-lg font-semibold">
              {horaEnBolivia(r.origen.hora)} <span className="text-slate-400">→</span> {horaEnBolivia(r.destino.hora)}
              <span className="ml-2 text-sm font-normal text-slate-500">{formatearDuracion(r.duracion_min)}</span>
            </p>
            <p className="text-sm text-slate-600">
              {r.origen.terminal} → {r.destino.terminal}
            </p>
