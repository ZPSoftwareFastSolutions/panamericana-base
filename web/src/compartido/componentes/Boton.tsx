...
type PropiedadesBoton = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  ...
  /** mientras es true, el boton se desactiva y muestra "Guardando..." */
  cargando?: boolean;
};

const ESTILOS = {
  principal: 'bg-slate-900 text-white hover:bg-slate-700',
  secundario: 'border border-slate-300 bg-white text-slate-900 hover:bg-slate-100',
};
...
export function Boton({
  ...
  return (
    <button
      ...
      className={`rounded px-4 py-2 font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${ESTILOS[variante]} ${className}`}
    >
